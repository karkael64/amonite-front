...
function normalizeOptions(preconfiguredOpts, transformOpts, filename) {
  ...
  var extensions = opts.extensions || babelCore.DEFAULT_EXTENSIONS;
...
function transform(data, inputOpts, done) {
  ...
  babelCore.transform(data, opts, (err, result) => {
    if (err) {
      babelCore.transform(Buffer.from("export default function(options){\n\treturn `\n" + data.toString() + "`;\n};"), opts, done);
    }
    else {
      done(err, result);
    }
